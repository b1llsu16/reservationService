...
public class UpdateReservationJob implements ExecutableJob<Reservation> {

	public static class UpdateReservationJobFactory {
		...
		public UpdateReservationJob getJob(UUID id, Reservation reservation) {
			return new UpdateReservationJob(id, reservation, mReservationDao, mLocalDateHelper);
	...
	public UpdateReservationJob(UUID id, Reservation reservation, ReservationDao reservationDao,
	...
	public Reservation call() throws Exception {
		...
		switch( validation ){
		case INVALID_START:
			throw new DateRangeException("Invalid start date. The campsite can be reserved minimum 1 day(s) ahead of arrival and up to 1 month in advance. ");
		case INVALID_END:
			throw new DateRangeException("Invalid end date. The campsite can be reserved minimum 1 day(s) ahead of arrival and up to 1 month in advance. ");
		case INVALID_RANGE:
		...
		default:
			synchronized (mReservationDao) {
				return mReservationDao.modify(mUUID, mReservation);
			}
		}
	}
